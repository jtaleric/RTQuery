...
class RTQuery(object):
    ...
    def __init__(self, config):
        self.CONFIG = config
        self.es = Elastic.Elastic(self.CONFIG['elastichost'],self.CONFIG['elasticport'])
    ...
    def _push(self,url, payload):
        krb = KerberosTicket.KerberosTicket(self.CONFIG['domain'])
        headers = {"Authorization": krb.auth_header}
        req = requests.get(url,headers=headers,params=payload,data=payload,verify=False)
        return req
    ...
    def rt_es_mapper(self, request):
        ...
        mapper = {'timestamp': parser.parse(request['Created']),
        ...
        elif 'Short Request Summary' in request:
            mapper['use'] = request['Short Request Summary']
        else :
            mapper['use'] = 'unknown'
        if 'Other products being tested' in request:
               mapper['other_use'] = request['Other products being tested']
        ...
        elif 'Primary product being tested' in request:
              mapper['product'] = request['Primary product being tested']
        else:
